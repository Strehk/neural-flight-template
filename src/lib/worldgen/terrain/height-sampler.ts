 ...
 *
 * The relief composition (everything except the high-frequency `detail`
 * term) is factored into `composeRelief` so the river routing surface
 * (`macroHeight`) can reproduce the real terrain shape — minus detail — and
 * therefore route water through actual valleys with water surfaces that
 * track the ground. The river *carve* itself happens in TerrainSampler,
 * after this stage, so this module stays free of any river imports.
 */
...
export interface ReliefContext {
  ...
  cliffNoise: number;
  mountainMass: number;
  /** Composed height WITHOUT the high-frequency `detail` term. */
  reliefHeight: number;
}

/**
 * Compose the smooth relief height — all contributors except the
 * high-frequency `detail` term. Shared by `sampleHeight` (which adds detail)
 * and the river routing surface (which wants the smooth shape only).
 */
export function composeRelief(
  biome: BiomeContext,
  noise: NoiseStack,
  biomeScale: number,
  ...
  return { rolling, ridgePrimary, cliffNoise, mountainMass, reliefHeight };
}
...
export function sampleHeight(
  ...
  const scale = biomeScale;
  const { wx, wz, forestWeight, mountainWeight, snowWeight, rugged } = biome;

  const { rolling, ridgePrimary, cliffNoise, mountainMass, reliefHeight } =
    composeRelief(biome, noise, biomeScale, mountainHeight);
  ...
  // High-frequency detail — the only term excluded from the relief surface.
  const detail =
