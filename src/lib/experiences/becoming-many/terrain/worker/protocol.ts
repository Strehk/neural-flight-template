...
//
// Plain-data messages across the worker boundary (no three / no class
// instances, so they structured-clone / transfer cleanly). The worker computes
// a chunk's per-vertex local position + normal arrays from the active provider
// and transfers them back; the main thread wraps them in a BufferGeometry.
