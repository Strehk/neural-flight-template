...
//
// A lean round-robin pool of terrain workers (modeled on the repo's
// WorldgenWorkerPool, but provider-agnostic). build() returns a promise for the
// chunk's vertex arrays; the ChunkScheduler handles cancellation of results that
// arrive after the player has moved (it disposes the chunk on resolve), so the
// pool itself stays minimal — no generation bookkeeping needed here.

import type { TerrainConfig } from "../provider";
import type { TerrainBuildResult, WorkerOutbound } from "./protocol";
// Vite '?worker' import — bundles terrain.worker.ts as a Web Worker.
...
export class TerrainWorkerPool {
	...
	private handleError(e: ErrorEvent): void {
		// A worker crashed; fail everything in flight (rare). The scheduler will
		// re-request missing chunks on the next update.
