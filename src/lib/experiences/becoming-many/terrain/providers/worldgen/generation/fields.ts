/**
 * CPU replacement for the original GPU field library (was gpu/tslNoise.ts).
 *
 * The source evaluated these as TSL `mx_noise_float` (gradient noise) compute
 * passes on the renderer. A Web Worker has no WebGPU device, so we re-express the
 * SAME structure (domain warp → continent fbm → land mask → ridged mountains;
 * climate + lapse; moisture) as pure JS using the repo's value-noise fBM. The
 * *look* differs from the GPU version (value noise ≠ gradient noise) — that is an
 * accepted trade (see the M4 plan) — but every function stays a pure function of
 * world position + seed, which is what keeps region/chunk borders seamless.
 *
 * Per-seed variation comes ONLY from a coordinate offset (`seedToOffset`), never
 * from reseeding the noise — identical to the original discipline, so two chunks
 * sampling a shared edge get identical values.
 */
import type { GenParams } from "./mapTypes";
import { signedFbm2D, valueNoise2D } from "./noise";
...
/** Signed fBM in ~[-1,1] (matches the original `fbm2`). */
function fbm(x: number, y: number, seed: number, octaves: number): number {
	return signedFbm2D(x, y, seed, octaves);
}

/** Ridged multifractal in ~[0,1] with sharp crests (matches the original `ridged2`). */
 ...
 * ranges, no fine detail. The macro grid samples exactly this, and the drainage
 * (rivers/lakes) is derived from it.
 */
...
export function temperature01(
	...
	const climate = fbm(psx / (P.continentScale * 1.5), psy / (P.continentScale * 1.5), S_CLIMATE, 3) * 0.5 + 0.5;
	const grad = fbm(psx / (P.continentScale * 4), psy / (P.continentScale * 4), S_GRAD, 2) * (P.temperatureGradient * 0.5);
...
export function baseMoisture01(
	...
	const m = fbm(psx / Math.max(200, P.moistureScale), psy / Math.max(200, P.moistureScale), S_MOIST, 4) * 0.5 + 0.5;
	const height = baseHeight01(wx, wy, seedOffset, P);
