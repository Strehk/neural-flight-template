 ...
 */
import type { GenParams, RiverNetwork, RiverPath, RiverPoint } from '../mapTypes';
import { flowToWidth, flowToDepth } from './RiverPath';
import { valueNoise2D } from '../noise';
import { emptyNetwork } from './RiverNetwork';
...
export function traceRivers(input: TraceInput): RiverNetwork {
  ...
  const visible = (i: number): boolean =>
    srcAllowed[i] === 1 || accum[i] >= threshold * visibleMul;
  ...
  for (const head of heads) {
    ...
    const emit = (i: number, depth: number): void => {
      const w = worldOf(i);
      const width = flowToWidth(accum[i], threshold, params);
      points.push({ x: w.x, y: w.y, flow: accum[i], width, depth: depth < 0 ? flowToDepth(width, params) : depth });
