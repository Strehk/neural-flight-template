/**
 * Caches per-region macro plans (+ drainage substrate, added in step 5).
 *
 * Regions are generated lazily and cached (bounded LRU). Because a chunk can
 * straddle up to 2×2 regions, callers fetch all regions overlapping a world rect
 * via `regionsForRect`, then look up macro tiles per pixel with `tileAt`.
 */
import type { MacroWorldGenerator } from '../MacroWorldGenerator';
import type { GenParams, RegionData } from '../mapTypes';
import { regionKey } from './WorldCoords';
...
export class RegionManager {
  ...
  tileAt(mx: number, my: number, regions: Map<string, RegionData>, params: GenParams): number {
    ...
    const lx = mx - rx * RM;
    const ly = my - ry * RM;
    return region.macroTiles[ly * RM + lx];
  }
