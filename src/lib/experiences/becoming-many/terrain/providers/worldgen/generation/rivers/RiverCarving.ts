 ...
 */
import type { ChunkData, GenParams, RiverNetwork } from '../mapTypes';
  ...
  const stamp = (cx: number, cy: number, width: number, depth: number): void => {
    ...
    for (let py = py0; py <= py1; py++) {
      ...
      for (let px = px0; px <= px1; px++) {
        ...
        if (rv > riverMap[idx]) riverMap[idx] = rv;
        const h = heightMap[idx] - depth * fall;
    ...
    for (const path of net.paths) {
      ...
      for (let s = 0; s < pts.length - 1; s++) {
        ...
        for (let t = 0; t <= steps; t++) {
          const f = t / steps;
          stamp(a.x + dx * f, a.y + dy * f, a.width + (b.width - a.width) * f, a.depth + (b.depth - a.depth) * f);
        }
      }
    }
  }
