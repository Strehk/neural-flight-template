...
import { heightToWorldY, type TerrainDetailGenerator } from "../terrain3d/TerrainDetailGenerator";
import type { TerrainSampler } from "../terrain3d/TerrainSampler";
import { waterVertexColor } from "./shoreline";
...
function buildStillWater(
	...
	for (let j = 0; j < res; j++) {
		for (let i = 0; i < res; i++) {
			...
			for (let c = 0; c < 4; c++) {
				if (f[c] > 0) poly.push({ gx: i + cgx[c], gy: j + cgy[c], depth: f[c] });
				const d = (c + 1) % 4;
				const fa = f[c];
				const fb = f[d];
				if (fa > 0 !== fb > 0) {
					const t = fa / (fa - fb);
					poly.push({
						gx: i + cgx[c] + (cgx[d] - cgx[c]) * t,
						gy: j + cgy[c] + (cgy[d] - cgy[c]) * t,
						depth: 0,
					});
				}
			}
			...
			for (let t = 1; t < poly.length - 1; t++) {
				pushVert(poly[0], levelY);
				pushVert(poly[t], levelY);
				pushVert(poly[t + 1], levelY);
			}
		}
	}
}
...
function buildRivers(
	...
	const inStillWater = (wx: number, wy: number): boolean => {
		...
		return waterMask[idx] === 1 && (heightMap[idx] < sea || lakeMap[idx] > 0.08);
	};
