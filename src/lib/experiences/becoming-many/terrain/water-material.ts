...
// material only shades it: animated ripple normals, a view-angle fresnel toward
// the sky, and a sun glint (ported from WolrdGen3's WaterSurfaceMaterial). On top
// of that it fades into the sense void with the SAME view bubble as the terrain
// (viewReveal over the shared sense uniforms), so water belongs to the current
// sense instead of glowing in the dark.
//
...
export function createWaterMaterial(u: KitUniforms, uTime: Node): MeshBasicNodeMaterial {
	const sunDir = uniform(vec3(0.4, 0.7, 0.4));
	...
	mat.opacityNode = clamp(float(0.72).add(fres.mul(0.24)).add(max(spec, float(0))), 0, 0.96).mul(
		reveal,
	);
