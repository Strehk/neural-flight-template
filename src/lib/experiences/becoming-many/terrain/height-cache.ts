...
export function makeHeightEntry(
	...
	return {
		heightGrid,
		originX: gridX * chunkSize,
		originZ: gridZ * chunkSize,
...
export function sampleEntry(e: HeightEntry, x: number, z: number): number {
	...
	const v00 = g[z0 * e.vpe + x0];
	const v10 = g[z0 * e.vpe + x1];
	const v01 = g[z1 * e.vpe + x0];
	const v11 = g[z1 * e.vpe + x1];
...
export class ChunkHeightCache {
	...
	add(gridX: number, gridZ: number, heightGrid: Float32Array): void {
		this.entries.set(`${gridX},${gridZ}`, makeHeightEntry(gridX, gridZ, this.chunkSize, heightGrid));
	}

	remove(gridX: number, gridZ: number): void {
		this.entries.delete(`${gridX},${gridZ}`);
	}
	...
	sample(x: number, z: number, fallback = 0): number {
		const gx = Math.floor(x / this.chunkSize);
		const gz = Math.floor(z / this.chunkSize);
		const e = this.entries.get(`${gx},${gz}`);
